...
namespace CMU462 {
    ...
    void SoftwareRendererImp::rasterize_image(float x0, float y0,
        ...
        //rasterizes image using mip map
        //sets up variables
        float dx = x1 - x0;
        float dy = y1 - y0;     
        ...
        for (int x = (int)floor(x0 * sample_rate); x <= (int)floor(x1 * sample_rate); x++) {
            for (int y = (int)floor(y0 * sample_rate); y <= (int)floor(y1 * sample_rate); y++) {  //for loop for y
                //finds u and v variable
                float u = (x * pd - x0) / dx;
                float v = (y  * pd - y0) / dy;
                //sets color
                Color c = sampler->sample_nearest(tex, u, v, 0);
                //rasterizes  point
                rasterize_point(x * pd, y * pd, c);
            }
        }
    }
